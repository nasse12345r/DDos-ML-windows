...
# Maps live cicflowmeter (Python) column names -> training dataset column names
...
def preprocess_flows(df):
    ...
    # Rename live columns to match training feature names
    df = df.rename(columns=CICFLOWMETER_RENAME_MAP)

    matched = [c for c in features if c in df.columns]
    missing = [c for c in features if c not in df.columns]
    logger.info(f"DEBUG PREPROCESS: {len(matched)}/{len(features)} feature columns matched.")
    if missing:
        logger.info(f"DEBUG PREPROCESS: Still missing: {missing}")
